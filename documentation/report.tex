...
The baseline achieves macro F1 of 0.25--0.27 across languages; somewhat
unexpectedly, both transformer models fall below this on macro F1 across all
three languages, suggesting that data volume rather than model complexity is
the primary bottleneck in this setting.
...
The core challenge we address is whether multilingual pre-trained models can
classify emotions in Bantu languages when labelled training data is very
limited and class imbalance is severe.
These languages are morphologically complex and largely absent from transformer
pre-training corpora, so it is not obvious that fine-tuning on a few hundred
examples will outperform a simpler bag-of-words approach.
Understanding when large models fail in genuinely low-resource settings is
important for deciding how to invest in NLP tooling for underserved African
language communities.

\paragraph{Research Questions.}
\begin{enumerate}[noitemsep]
  \item \textit{Model comparison:} How do multilingual transformer models
        (mBERT, XLM-R) compare to a TF-IDF baseline for multi-label emotion
        classification in Bantu languages, and does per-label class weighting
        improve recall on rare emotion categories?
  \item \textit{Cross-lingual variation:} How does performance vary across
        isiZulu, isiXhosa, and Swahili, and does having a proper training
        split (as Swahili does) make a meaningful difference compared to
        using the development set as a training surrogate?
  \item \textit{Error patterns:} What kinds of prediction errors are most
        common, which emotion classes are systematically hardest to predict,
        and what do these patterns reveal about model behaviour under
        severe data scarcity and class imbalance?
\end{enumerate}

\paragraph{Contributions.}
The main contributions of this work are:
(1) a reproducible multi-label emotion classification pipeline for three Bantu
languages that can serve as a reference implementation for future work;
(2) a systematic comparison of TF-IDF and two multilingual transformer models
under matched, controlled evaluation conditions across three low-resource
languages; and
(3) an analysis of class imbalance effects, error patterns, and cross-lingual
performance variation that provides practical insights for NLP development
targeting underrepresented African languages.
...
mBERT in particular has shown strong zero-shot transfer even to languages not
explicitly represented during pre-training, which is why it remains a common
first choice when evaluating new low-resource languages \citep{devlin2019bert}.
...
A key quality of both benchmarks is that they use native-speaker annotations,
which matters for emotion tasks where cultural and linguistic context strongly
influences label interpretation---a standard that BRIGHTER also follows.
...
The use of fluent native speakers for annotation is a particular strength:
many multilingual datasets rely on translation or non-native annotators, which
can introduce noise especially for culturally variable emotions like disgust
and fear.
...
\paragraph{Responsible NLP.}
A recurring concern in African language NLP is that research priorities are
often shaped by data availability rather than community need, and the
languages covered by benchmarks reflect where annotation was most feasible
rather than where NLP support is most urgently needed.
The pre-training corpora for mBERT and XLM-R are also heavily dominated by
high-resource European languages, which means their shared subword vocabularies
may not adequately represent Bantu morphology.
We flag these structural limitations as important context when interpreting
our results, and we note that performance differences between languages in
our study likely reflect data constraints as much as anything linguistic.
...
\paragraph{Responsible NLP.}
BRIGHTER is a publicly released research dataset and we do not use any
proprietary, personally identifiable, or copyrighted material.
Using the development split as training data for isiZulu and isiXhosa is a
deliberate compromise to use all available labelled data; we acknowledge that
this blurs the train--validation boundary and may underestimate what a proper
training partition would achieve.
Class imbalance was addressed through inverse-frequency weighting rather than
discarding samples, so that all available training signal for minority emotion
...
library \citep{wolf-etal-2020-transformers} under identical, controlled
conditions to ensure fair comparison: 3 epochs, learning rate
$2 \times 10^{-5}$,
...
\paragraph{Error analysis methodology.}
To directly address RQ3, we manually inspect test-set predictions and
categorise errors as: false negatives (model predicts nothing despite a
positive ground-truth label), over-predictions (model predicts a label
absent from the ground truth), or partial matches (some labels correct,
others missed or spurious).
We additionally flag any per-class F1 below 0.15 as a \textit{weak label},
indicating a systematic failure to learn that emotion class.
...
per language.
This is worth contextualising against prior literature.
\citet{mabokela2024explainable} generally find that pre-trained models
outperform classical approaches for African language tasks, but those
experiments typically involve thousands of training examples.
We think the key difference here is data scale: with fewer than 900 samples
for isiZulu and isiXhosa, the transformers simply do not have enough signal
to recover from the class imbalance, even with weighted loss.
This also sits within the pattern described by \citet{thangaraj2024crosslingual},
who note that XLM-R's advantage over mBERT is most visible when there is
enough training data to support the larger model---a threshold we clearly
did not reach for the two languages without official training splits.
...
We were also glad to find that class weighting prevented total collapse to
majority-class predictions.
In early experiments without \texttt{pos\_weight}, the transformer models
just predicted all-zeros on minority classes, which made evaluation
meaningless.
Getting the weighting right---even though it hurt precision---was a necessary
step to produce interpretable results across all six emotion classes.
...
\texttt{pos\_weight} trade-off between sensitivity and specificity.
...
Future work should explore several concrete directions.
Data augmentation---for example, back-translating training examples through
English, or projecting labelled data from Swahili into isiZulu and isiXhosa
via cross-lingual transfer---would directly target the training data shortage
that constrained all our models.
Fine-tuning AfroXLMR \citep{alabi2022adapting}, which was specifically adapted
for African languages, is the most logical next model to test; it likely
handles Bantu morphology better than generic mBERT and XLM-R.
Per-class threshold tuning rather than a fixed $\tau = 0.3$ for all labels
could also improve the precision--recall trade-off we observed, particularly
for rare emotions like disgust and fear.
Finally, including the EthioEmo dataset for Amharic would allow a controlled
comparison between Bantu and non-Bantu African languages, directly testing
whether genealogical relatedness aids cross-lingual transfer beyond what
data availability alone can explain.
