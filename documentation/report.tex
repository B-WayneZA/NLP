...
\author{YM (Malaika) Kamangu \\  Department of Computer Science \\ University of Pretoria \\ \texttt{u23618583@tuks.co.za}
  \And
  NL (Lebogang) Masenya \\ Department of Computer Science \\ University of Pretoria \\ \texttt{u236747483@tuks.co.za}
  \And
  DK (David) Musa-Aisien \\ Department of Computer Science \\ University of Pretoria \\ \textbf{Group 12} --- COS 760 S1 2026 \\ \texttt{u23647374@tuks.co.za}}
...
Macro F1 scores range from 0.247 (Swahili) to 0.265 (isiXhosa).
...
However, Swahili achieves a notably lower micro F1 (0.293), suggesting its
...
Swahili  & 0.247 & 0.293 & 0.214 & 0.298 \\
\bottomrule
\end{tabular}
...
The mBERT results reveal a strong training-data dependence.
isiXhosa (682 training samples) achieves macro F1 of 0.244 with mBERT,
broadly comparable to its TF-IDF baseline (0.265), while micro F1 improves
markedly from 0.471 to 0.536 --- indicating better calibration on frequent
classes.
isiZulu (875 training samples) shows lower macro F1 with mBERT (0.082)
than the TF-IDF baseline (0.257), suggesting that the transformer's
inductive biases are harder to activate with fewer than ~900 labelled examples
in a multi-label setting.
We attribute this result partly to the data constraint and partly to the
severe sparsity of the minority emotion classes (disgust at 2.9\%,
fear at 3.1\% of isiZulu training labels).
Swahili, with the largest training set (3{,}307 examples), achieves macro
F1 of 0.145 --- slightly better than isiZulu but still below its TF-IDF
baseline of 0.248.  The high precision (0.254) relative to recall (0.153)
suggests the model is conservative, only predicting emotions when it is
confident, which suppresses recall on minority classes.
...
\textbf{Language} & \textbf{F1-mac} & \textbf{F1-mic} & \textbf{Prec-mac} &
  \textbf{Rec-mac} \\
\midrule
isiZulu  & 0.082 & 0.320 & 0.065 & 0.109 \\
isiXhosa & 0.244 & 0.536 & 0.213 & 0.291 \\
Swahili  & 0.145 & 0.287 & 0.254 & 0.153 \\
\bottomrule
\end{tabular}
...
\textbf{Language} & \textbf{F1-mac} & \textbf{F1-mic} & \textbf{Prec-mac} &
  \textbf{Rec-mac} \\
\midrule
isiZulu  & 0.000 & 0.000 & 0.000 & 0.000 \\
isiXhosa & 0.170 & 0.486 & 0.146 & 0.235 \\
Swahili  & 0.184 & 0.332 & 0.235 & 0.188 \\
\bottomrule
\end{tabular}
...
XLM-R produces a different pattern than mBERT.
For isiZulu (875 samples), XLM-R collapses to all-zero predictions at every
threshold, whereas mBERT at least achieves macro F1 of 0.082.
XLM-R has 278M parameters vs.\ mBERT's 178M, making it more data-hungry
to fine-tune; with under 900 labelled examples the model cannot escape
a degenerate zero-prediction solution.
For isiXhosa (682 samples), XLM-R achieves macro F1 of 0.170, lower than
mBERT's 0.244 but with a competitive micro F1 of 0.486.
The reversal occurs for Swahili: XLM-R (F1 = 0.184) outperforms mBERT
(F1 = 0.145) when trained on 3{,}307 examples, confirming that XLM-R
benefits more from additional training data than mBERT does.
Notably, the TF-IDF baseline (F1 = 0.247--0.265) outperforms both
...
The TF-IDF + Logistic Regression baseline achieves macro F1 of 0.247--0.265,
...
(3) the TF-IDF baseline outperforms both transformer models on all three
languages under the data constraints of this study.

For the transformer experiments, mBERT and XLM-R exhibit complementary
data-sensitivity profiles.
mBERT (178M parameters) generalises better in the lowest-data conditions
(isiZulu: F1 = 0.082; isiXhosa: F1 = 0.244), whereas XLM-R (278M parameters)
achieves higher macro F1 on Swahili (0.184 vs.\ mBERT's 0.145) once 3{,}307
training examples are available.
XLM-R completely fails on isiZulu (F1 = 0.000), predicting no positive
labels at any threshold---a direct consequence of attempting to fine-tune
278M parameters on fewer than 900 labelled instances.
